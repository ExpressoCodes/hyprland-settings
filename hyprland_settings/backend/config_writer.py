...
def strip_section_from_main_config(section_name: str, main_config_path: Path) -> None:
    """Remove managed hl.config/hl.animation blocks for *section_name* from main config.

    Called once (on first launch) after the section file is created, so the
    main config no longer duplicates settings that now live in the section file.
    Backs up the file before modifying. No-op for formats other than Lua.
    """
    if detect_format(main_config_path) != ConfigFormat.LUA:
        return

    # Keys managed per section: top-level hl.config table keys to strip
    _SECTION_KEYS: dict[str, list[str]] = {
        "appearance": ["general", "decoration"],
        "animations":  ["animations"],
        "input":       ["input"],
    }
    managed_keys = _SECTION_KEYS.get(section_name)
    if not managed_keys:
        return

    _backup_once(main_config_path)

    text = main_config_path.read_text(encoding="utf-8")
    original = text

    # Strip hl.config({...}) blocks whose top-level key is one we manage.
    # Strategy: find each hl.config({ ... }) call, check its first key, drop it if managed.
    # We match balanced braces so nested tables are handled correctly.
    result_parts: list[str] = []
    pos = 0
    pattern = re.compile(r'hl\.config\s*\(\s*\{')
    for m in pattern.finditer(text):
        start = m.start()
        ...
        while i < len(text):
            ...
            if ch == '{':
                ...
                if depth == 0:
                    ...
                    # Check whether this hl.config block's first key is a managed one
                    body_start = m.end()  # after 'hl.config({'
                    # Extract first identifier key
                    key_m = re.match(r'\s*(\w+)\s*=', text[body_start:j])
                    first_key = key_m.group(1) if key_m else ""
                    if first_key in managed_keys:
                        # Skip the block — also consume one trailing newline
                        end = j + 1
    ...
    # Strip hl.animation(...) lines when stripping the animations section
    if section_name == "animations":
        new_text = re.sub(r'hl\.animation\s*\([^)]*\)\n?', '', new_text)

    if new_text == original:
        log.info("No managed blocks found in main config for section '%s'", section_name)
        return

    # Collapse runs of 3+ blank lines to 2
    new_text = re.sub(r'\n{3,}', '\n\n', new_text)

    tmp = main_config_path.with_suffix(main_config_path.suffix + ".tmp")
    tmp.write_text(new_text, encoding="utf-8")
    os.replace(tmp, main_config_path)
    log.info("Stripped '%s' blocks from %s", section_name, main_config_path)
